...
receive_pipeline (void)
{
    ...
    pipe1 = gst_parse_launch ("tee name=audiotee ! queue ! fakesink "
    ...
    g_print ("WEBRTC TEST: Starting pipeline, not transmitting yet\n");
    ...
    err:
    g_print ("WEBRTC TEST: State change failure\n");
    if (pipe1)
...
start_negotiation (void)
{
     ...
     * inside the same pipeline. We start by connecting it to a fakesink so that
     * we can preroll early. */
    pipe1 = gst_parse_launch ("tee name=sendrecv ! queue ! fakesink "
                                 "audiotestsrc is-live=true wave=red-noise ! queue ! opusenc ! rtpopuspay ! "
...
handle_sdp_offer (const gchar * peer_id, const gchar * text)
{
    ...
    promise = gst_promise_new_with_change_func (
            (GstPromiseChangeFunc) on_answer_created, (gpointer) "sendrecv", NULL);
...
start_pipeline (void)
{
    ...
    g_signal_emit_by_name (webrtc1, "create-data-channel", "channel", NULL,
                           &send_channel);
    if (send_channel) {
        g_print ("WEBRTC TEST: Created data channel\n");
        connect_data_channel_signals (send_channel);
    } else {
        g_print ("WEBRTC TEST: Could not create data channel, is usrsctp available?\n");
    }
